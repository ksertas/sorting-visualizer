...
export default function MobileOptionsBar() {
  ...
  return (
    <div className='absolute bottom-8 w-screen lg:static lg:w-72 lg:mt-8 lg:ml-8'>
      <div>
        <div className={`bg-container-dark ${activeMenu === 'ALGORITHMS' ? '' : 'hidden'} rounded-sm min-h-24 w-10/12 mx-auto mb-4 px-8 py-4 lg:block lg:m-0 lg:w-full`}>
          <h4 className='text-text-gray text-base font-bold pb-4'>ALGORITHM</h4>
          <AlgorithmList />
        </div>
        <div className={`bg-container-dark ${activeMenu === 'SORTING OPTIONS' ? '' : 'hidden'} rounded-sm min-h-24 w-10/12 mx-auto mb-4 px-8 py-4 lg:block lg:m-0 lg:w-full`}>
